...
`releaseMatrixGate` runs the shared tests, all four three-loader modern family gates, the cleanup-aware GameTest gate, the isolated Forge 1.7.10 Java 8 gate, centralized inspection of every final JAR, and a fresh dedicated-server check for all 13 targets. Each target must first reject an invalid canonical config without leaking its value, then start successfully and complete authenticated library and sonic-capability calls against a deterministic loopback Plex service. Every modern target must also reject a protocol client that has neither its loader nor Jammarr; the legacy Forge missing-mod handshake remains a real-client acceptance item. The gate places exactly 13 runtime artifacts in `build/releases/` alongside `manifest.json` and `SHA256SUMS` and fails if a tested server leaves a process or game port behind.
