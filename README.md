...
PAmpMod provides one server-authoritative Plex music queue for a NeoForge Minecraft server. Players browse and queue music in-game, operators control playback, and every listening client follows the same server timeline.

PAmpMod is intentionally a required server-and-client mod. A server-only mod cannot decode arbitrary Plex audio through an unmodified Minecraft client's sound engine.

## Requirements

- Minecraft 1.21.1, NeoForge 21.1, and Java 21.
- The same PAmpMod JAR on the dedicated server and every connecting client.
- A Plex Media Server reachable from the Minecraft server.

No external FFmpeg installation is required. Plex prepares an MP3 rendition; if a Plex version returns variable-bitrate data, PAmpMod normalizes it in-process to the configured constant bitrate using its bundled pure-Java encoder.

## Server setup

1. Start the server once to generate `world/serverconfig/pampmod-server.toml`.
...
3. Supply the token through `PAMPMOD_PLEX_TOKEN` (recommended), or put it in `plexToken` in the server config.
4. Restart the server after editing its server config. `/pamp reload` reruns connection and library validation against the currently loaded values.
...
- Press `P` or run `/pamp` to open the music screen.
...
- Each player may independently mute PAmpMod and set a persistent local volume. Local opt-out never changes the global queue.
- Master, Music, and PAmpMod volume controls all apply. Vanilla background music is suppressed while the PAmpMod stream is active and restored afterward.

Commands:

| Command | Access | Effect |
| --- | --- | --- |
| `/pamp` | Everyone | Open the screen |
| `/pamp status` | Everyone | Show global playback status |
| `/pamp pause`, `resume`, `skip`, `clear` | Operator | Control global playback |
| `/pamp cache` | Operator | Show cache usage |
| `/pamp reload` | Operator | Revalidate Plex and the selected library |
| `/pamp diagnostics` | Operator | Show sanitized Plex/cache/transfer state |
...
- Prefer `PAMPMOD_PLEX_TOKEN` so the token is not written to a configuration file.
- Library requests authenticate by header. Plex's progressive transcode endpoint requires query authentication on some server versions; PAmpMod never logs that request URI.
...
- PAmpMod does not modify Plex ratings, play history, or playlists.
...
```bash
PAMPMOD_LIVE_TEST=true \
PAMPMOD_PLEX_URL='https://plex.example.invalid:32400' \
PAMPMOD_PLEX_TOKEN='...' \
./gradlew test --tests stonytark.pampmod.server.PlexLiveSmokeTest
```
