...
- Press `J` by default on modern Minecraft (`P` on 1.7.10 and 1.8.9), or run `/jammarr` to open the music screen. The key appears as **Open Jammarr** in Controls and can be rebound on every supported version and loader. The modern default avoids vanilla's Social menu binding on `P`.
