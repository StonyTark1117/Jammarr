...
Jammarr 1.1.0 currently produces 70 artifacts covering 94 loader/version runtimes across 28 Minecraft versions. Jammarr is optional on clients: an unmodified client can remain on a Jammarr server but receives no browsing, controls, or audio. A participating client must use the same Minecraft version and a protocol-compatible Jammarr build. Supported Quilt installations use the matching Fabric artifact.
...
| 1.8.9 | 8 | unavailable | preview | unavailable |
