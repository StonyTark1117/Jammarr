...
Jammarr 1.0.0 produces 16 required client-and-server artifacts covering 21 supported loader/version runtimes. The server and every client must use the same Minecraft version, loader, and Jammarr version. Supported Quilt installations use the matching Fabric artifact.
...
Fabric, Quilt, and NeoForge are unavailable for Minecraft 1.7.10. Every modern Fabric artifact declares `fabricloader >=0.19.2`, which Quilt Loader 0.30.0 satisfies; ordinary Fabric builds remain pinned to Fabric Loader 0.19.3. The non-obfuscated 26.x targets include an idempotent Quilt bootstrap because Quilt Loader discovers Fabric mods there without dispatching their Fabric entrypoints. QSL and Quilted Fabric API are not required. Unmodded clients, Fabric-to-Quilt clients, other cross-loader clients, cross-Minecraft clients, and worlds downgraded through unsupported vanilla world formats are not supported.
