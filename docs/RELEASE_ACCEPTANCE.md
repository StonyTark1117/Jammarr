...
The release gate observes each real client's decoded output through an isolated audio sink; PLAYING state or an allocated OpenAL source alone is not accepted. Its deterministic carrier includes alternating timing markers every 250 ms, and the retained JSON reports onset, marker displacement, silence gaps, and inter-client skew. It also starts every modern Fabric artifact under Loader 0.19.2 as well as the pinned 0.19.3, and repeats all five Quilt client/audio scenarios with the pinned Mod Menu installed. Retain a result for every loader before publishing a Minecraft family:
...
Every clean two-client run must capture at least ten seconds with no post-start silence gap over 60 ms, marker displacement over 40 ms, onset over 300 ms (one 250 ms marker period plus the analysis window), or inter-client skew over 150 ms. Run `scripts/run-audio-impairment-matrix.sh` for the representative 1.7.10 Forge plus 1.20.1 and 26.2 Fabric, Quilt, Forge, and NeoForge targets. Retain all six profiles: direct, fixed 150 ms latency, 20–250 ms jitter, one two-second stall, repeated 250 ms client stalls, and six seconds below the configured stream bitrate.
