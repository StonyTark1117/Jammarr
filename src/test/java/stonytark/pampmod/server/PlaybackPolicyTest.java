...
class PlaybackPolicyTest {
    @Test void appliesEveryRestartMode() {
        assertEquals(new RestartPolicy.Restoration(false, 0, false), RestartPolicy.restore(PampConfig.RestartMode.RESTART_TRACK, 4_000, true));
        assertEquals(new RestartPolicy.Restoration(true, 0, false), RestartPolicy.restore(PampConfig.RestartMode.CLEAR, 4_000, true));
        assertEquals(new RestartPolicy.Restoration(false, 4_000, true), RestartPolicy.restore(PampConfig.RestartMode.RESUME_POSITION, 4_000, true));
    }
