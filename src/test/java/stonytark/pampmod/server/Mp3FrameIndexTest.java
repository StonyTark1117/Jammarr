...
class Mp3FrameIndexTest {
    @Test void splitsOnlyOnFrameBoundariesAndFindsTime() {
        ...
        assertTrue(chunks.stream().allMatch(c -> c.data().length <= Mp3FrameIndex.MAX_CHUNK_BYTES));
        assertTrue(chunks.stream().allMatch(c -> Hashing.matchesSha256(c.data(), c.sha256())));
    ...
    @Test void ignoresId3v2Prefix() {
        ...
        assertEquals(frame.length, Mp3FrameIndex.split(stream).getFirst().data().length);
    }

    @Test void rejectsNonMp3Responses() {
        assertThrows(IllegalArgumentException.class, () -> Mp3FrameIndex.split("not audio".getBytes()));
    }
