...
public final class JammarrClientConfigScreen extends Screen {
    ...
    @Override protected void init() {
        ...
        addRenderableWidget(Button.builder(listeningLabel(), button -> {
            JammarrConfig.ENABLED.set(!JammarrConfig.ENABLED.get());
            JammarrConfig.ENABLED.save();
    ...
    private static Component listeningLabel() {
        return Component.translatable("jammarr.config.listening", Component.translatable(JammarrConfig.ENABLED.get() ? "jammarr.config.on" : "jammarr.config.off"));
    }
    ...
    private static final class VolumeSlider extends AbstractSliderButton {
        private VolumeSlider(int x, int y, int width, int height) {
            super(x, y, width, height, Component.empty(), JammarrConfig.VOLUME.get());
            updateMessage();
        }
