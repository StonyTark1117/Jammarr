...
public final class JammarrAudioPlayer {
    ...
    // The shared-clock recorder can resolve a real 40 ms output displacement,
    // but OpenAL's queued-buffer position can move by roughly one mixer period
    // without the rendered stream moving with it. Rebuffering on that
    // diagnostic boundary turns a harmless clock-accounting discontinuity
    // into seconds of silence. Keep disruptive recovery aligned with the
    // independently enforced inter-client tolerance instead.
    ...
    private volatile PcmAudioStream pcmStream;
    ...
    public void tick() {
        ...
        if (decoder.canAcceptWindow(chunksPerRequest)) window.request(
                now, decoder.bufferedMillis(), StreamingMp3Decoder.MAX_BUFFERED_MS).ifPresent(request -> {
        ...
        if (JammarrSettings.enabled()) minecraft.getSoundManager().stop(null, SoundSource.MUSIC);
    ...
    private void startChannel(long now) {
        ...
        access.createHandle(Library.Pool.STREAMING).whenComplete((handle, error) -> {
            ...
            handle.execute(value -> {
                ...
                PcmAudioStream startingStream = new PcmAudioStream(startingDecoder);
    ...
    private void scheduleBackendProbe(long now) {
        ...
        PcmAudioStream activeStream = pcmStream;
    ...
    public void acceptanceClockDrift() {
        if (!ProtocolLimits.audioProbeEnabled() || manifest == null || !started) return;
        // Shift the common channel position so a capable OpenAL backend proves
        // the device-clock path first; the wall-clock policy remains the
        // bounded fallback when AL_SOFT_source_latency is unavailable.
    ...
    private static final class BackendPosition {
        ...
        private BackendPosition(PcmAudioStream stream, long observedAtMs, long observedAtMonotonicMs,
