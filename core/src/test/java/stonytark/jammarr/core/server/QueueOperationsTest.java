...
class QueueOperationsTest {
    @Test void appendsOnlyAvailableCapacity() {
        List<QueueTrack> queue = new ArrayList<QueueTrack>(Arrays.asList(track("1")));
        QueueOperations.AppendResult result = QueueOperations.append(queue, Arrays.asList(track("2"), track("3")), 2);
        assertEquals(QueueOperations.Result.APPLIED, result.result()); assertEquals(1, result.accepted()); assertEquals(2, queue.size());
        assertEquals(QueueOperations.Result.FULL, QueueOperations.append(queue, Arrays.asList(track("4")), 2).result());
    }

    @Test void restrictsMutationAndProtectsCurrentTrackFromReorder() {
        List<QueueTrack> queue = new ArrayList<QueueTrack>(Arrays.asList(track("1"), track("2"), track("3")));
