...
public final class JammarrCommands {
    @SubscribeEvent public void register(RegisterCommandsEvent event) {
        ...
        root.then(Commands.literal("reload").requires(s -> JammarrPermissions.has(s, JammarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); if (p != null) p.validatePlex(); c.getSource().sendSuccess(() -> Component.literal("Jammarr Plex validation started"), false); return 1; }));
        root.then(Commands.literal("cache").requires(s -> JammarrPermissions.has(s, JammarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); long bytes = p == null ? 0 : p.cacheSize(); c.getSource().sendSuccess(() -> Component.literal("Jammarr cache: " + bytes / 1024 / 1024 + " MiB"), false); return 1; }));
        root.then(Commands.literal("diagnostics").requires(s -> JammarrPermissions.has(s, JammarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); c.getSource().sendSuccess(() -> Component.literal(p == null ? "Jammarr is unavailable" : p.diagnostics()), false); return 1; }));
        root.then(Commands.literal("station").requires(s -> JammarrPermissions.has(s, JammarrSettings.operatorPermissionLevel()))
        ...
        root.then(Commands.literal("autoplay").requires(s -> JammarrPermissions.has(s, JammarrSettings.operatorPermissionLevel()))
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> operator(String name, JammarrPayloads.ControlAction action) {
        return Commands.literal(name).requires(s -> JammarrPermissions.has(s, JammarrSettings.operatorPermissionLevel())).executes(c -> {
            JammarrServer.instance().control(c.getSource().getPlayerOrException(), new JammarrPayloads.ControlRequest(action, -1)); return 1;
        });
    }

    private static int station(CommandSourceStack source, JammarrPayloads.StationAction action, JammarrPayloads.StationType type, boolean enabled)
        ...
        player.station(source.getPlayerOrException(), new JammarrPayloads.StationRequest(action, type, enabled, player.stationGeneration(), List.of()));
        return 1;
    }
}
