...
/** Schema-4 world state stored through Minecraft 1.16.5 WorldSavedData. */
public final class LegacySavedData extends WorldSavedData implements PlaybackStore {
    ...
    /**
     * PlaybackStore deliberately avoids WorldSavedData.setDirty's MCP name:
    ...
    public void load(CompoundNBT tag) {
        ...
        ListNBT queueTags = tag.getList("queue", 10);
        ...
        if (schema < 2) {
            ...
            ListNBT historyTags = tag.getList("history", 10);
    ...
    public CompoundNBT save(CompoundNBT tag) {
        ...
        ListNBT historyTags = new ListNBT();
    ...
    private static CompoundNBT writeTrack(QueueTrack track) {
    ...
    private static QueueTrack readTrack(CompoundNBT tag) {
    ...
    private static CompoundNBT writeStation(StationModels.StationDefinition station) {
        CompoundNBT tag = new CompoundNBT();
    ...
    private static StationModels.StationDefinition readStation(CompoundNBT tag) {
        ...
        ListNBT values = tag.getList("seeds", 10);
        for (int index = 0; index < Math.min(5, values.size()); index++) {
            CompoundNBT value = values.getCompound(index);
