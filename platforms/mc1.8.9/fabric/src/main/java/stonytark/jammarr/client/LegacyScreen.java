...
final class LegacyScreen extends Screen {
    ...
    @Override public void init() {
        ...
        if (view == View.SEARCH || view == View.ADVENTURE) {
            search = new TextFieldWidget(0, textRenderer, left, top, panel - 66, 20);
    ...
    @Override public void render(int mouseX, int mouseY, float tickDelta) {
        ...
        for (Object item : buttons) {
            ...
            if (tooltip != null && button.isMouseOver(client, mouseX, mouseY)) {
                renderTooltip(Collections.singletonList(tooltip), mouseX, mouseY);
                break;
            }
        }
    }
    ...
    private String trim(String value, int pixels) { return textRenderer.trimToWidth(value, Math.max(10, pixels)); }
