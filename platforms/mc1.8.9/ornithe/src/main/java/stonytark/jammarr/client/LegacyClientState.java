...
final class LegacyClientState implements LegacyNetwork.ClientListener {
    ...
    @Override public void accept(LegacyPacketTypes.Type<?> type, Object message) {
        ...
        if (type == LegacyPacketTypes.OPEN_SCREEN) {
            minecraft.openScreen(new LegacyScreen(this));
            ...
            if (hello.protocolVersion() != ProtocolLimits.clientHelloVersion() && minecraft.getNetworkHandler() != null) {
                minecraft.getNetworkHandler().getConnection().disconnect(
                        new LiteralText("Jammarr protocol mismatch: server requires version "
            ...
            if (minecraft.player != null) minecraft.player.addMessage(
                    new LiteralText("Jammarr: " + notice));
    ...
    void tick() {
        ...
        if (serverHelloReceived && ProtocolLimits.commandProbeEnabled() && !commandProbeSent
            ...
            Minecraft.getInstance().player.sendChat("/jammarr status");
            Minecraft.getInstance().player.sendChat("/jammarr diagnostics");
    ...
    private void runAcceptanceScreenProbe() {
        ...
        if (!acceptanceScreenLogged) {
            if (!(minecraft.screen instanceof LegacyScreen)) return;
        ...
        if (!acceptanceConfigScreenOpened) {
            minecraft.openScreen(new LegacyClientConfigScreen(minecraft.screen));
            acceptanceConfigScreenOpened = true;
            return;
        }
        if (!acceptanceConfigScreenLogged && minecraft.screen instanceof LegacyClientConfigScreen
    ...
    private void requestTimeSync() {
        if (Minecraft.getInstance().getNetworkHandler() == null) return;
        long now = System.currentTimeMillis();
