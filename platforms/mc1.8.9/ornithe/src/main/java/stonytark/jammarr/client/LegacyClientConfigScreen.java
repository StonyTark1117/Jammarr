...
public final class LegacyClientConfigScreen extends Screen {
    ...
    @Override protected void buttonClicked(ButtonWidget button) {
        if (button.id == 0) { minecraft.openScreen(parent); return; }
        if (button.id == 1) {
    ...
    @Override public void render(int mouseX, int mouseY, float tickDelta) {
        renderBackground();
        drawCenteredString(textRenderer, "Jammarr Client Settings", width / 2, height / 2 - 66, 0xFFFFFF);
        drawCenteredString(textRenderer, "These settings affect only this Minecraft client", width / 2, height / 2 - 50, 0xA0D8FF);
        drawCenteredString(textRenderer, "Volume " + Math.round(JammarrSettings.volume() * 100.0) + "%", width / 2, height / 2 + 6, 0xCFCFCF);
