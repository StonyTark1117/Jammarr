...
/** Compact eight-tab Forge 1.8.9 screen with full shared-control flows. */
final class LegacyScreen extends GuiScreen {
    ...
    @Override public void initGui() {
        ...
        if (view == View.SEARCH || view == View.ADVENTURE) {
            search = new GuiTextField(0, fontRendererObj, left, top, panel - 66, 20);
    ...
    @Override public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRendererObj, "Jammarr", width / 2, 12, 0xFFFFFF);
        ...
        drawCenteredString(fontRendererObj, trim(now, width - 20), width / 2, 24,
    ...
    private void drawNow(int left, int top, int panel) {
        ...
        drawCenteredString(fontRendererObj, empty(value.title()) ? "Nothing playing" : value.title(), width / 2, top + 12, 0xFFFFFF);
        if (!empty(value.artist())) drawCenteredString(fontRendererObj, value.artist(), width / 2, top + 28, 0xCFCFCF);
        if (!empty(value.sourceName())) drawCenteredString(fontRendererObj, "Source: " + value.sourceName(), width / 2, top + 44, 0xA0D8FF);
        drawCenteredString(fontRendererObj, "Audio: " + state.audioStatus(), width / 2, top + 66,
    ...
    private void drawStations(int left, int top, int panel) {
        ...
        drawString(fontRendererObj, "Sonic: " + value.capability().name().toLowerCase() + " - "
                + value.capabilityMessage(), left, top, 0xA0D8FF);
        drawString(fontRendererObj, value.active() && value.stationType() != StationModels.StationType.SONIC_ADVENTURE
                ? "Active: " + value.name() : "No general station active", left, top + 16, 0xCFCFCF);
        drawString(fontRendererObj, "Mix seeds: " + mixSeeds.size() + "/5", left, top + 28, 0xCFCFCF);
        int y = top + 72;
        for (StatePackets.QueueEntry entry : value.preview()) {
            drawString(fontRendererObj, trim(entry.title() + " - " + entry.artist(), panel), left, y, 0xBFCFFF); y += 14;
        }
    }

    private void drawAdventure(int left, int top, int panel) {
        drawString(fontRendererObj, "Sonic Adventure waypoints (2-5)", left, top, 0xA0D8FF);
        for (int index = 0; index < waypoints.size(); index++) {
            StationModels.StationSeed seed = waypoints.get(index);
            drawString(fontRendererObj, (index + 1) + ". " + trim(seed.title() + " - " + seed.subtitle(), panel - 30), left, top + 18 + index * 20, 0xCFCFCF);
        }
        int resultsTop = top + Math.min(5, waypoints.size()) * 20 + 70;
        if (!empty(state.adventure().message())) {
            drawString(fontRendererObj, state.adventure().message(), left, resultsTop, 0xBFCFFF); resultsTop += 14;
        }
    ...
    private void drawResults(int left, int top, int panel) {
        ...
        for (int row = 0; row < Math.min(MAX_VISIBLE_ROWS, results.items().size()); row++) {
            ...
            drawString(fontRendererObj, trim(prefix + item.title()
