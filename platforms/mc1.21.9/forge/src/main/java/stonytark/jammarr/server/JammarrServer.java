...
public final class JammarrServer {
    ...
    public void started(ServerStartedEvent event) {
        ...
        try {
            ...
            // Forge 59 starts its watchdog before ServerStartedEvent. Mark the
            // server stopped, wait for the normal save/close path, then force a
            // non-zero JVM exit because Forge 59 leaves a non-daemon scheduler
            // alive after an early dedicated-server shutdown.
            Jammarr.LOGGER.error("Unable to initialize Jammarr; stopping the server", error);
            Thread shutdown = new Thread(() -> {
                event.getServer().halt(true);
                System.exit(1);
            }, "Jammarr startup-failure shutdown");
    ...
    public void hello(ServerPlayer sender, JammarrPayloads.ClientHello payload) {
        if (capabilities.accept(sender.getUUID(), stonytark.jammarr.network.JammarrNetwork.PROTOCOL,
                stonytark.jammarr.network.JammarrNetwork.PROTOCOL) && player != null) player.hello(sender, payload.features(), payload.audioChunkBytes(), payload.chunksPerRequest());
    }
