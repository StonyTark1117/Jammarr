...
/** Compact eight-tab Beta 1.7.3 screen with full shared-control flows. */
final class LegacyScreen extends Screen {
    ...
    @Override public void init() {
        ...
        if (view == View.SEARCH || view == View.ADVENTURE) {
            search = new TextFieldWidget(this, textRenderer, left, top, panel - 66, 20, searchQuery);
    ...
    @Override protected void keyPressed(char character, int key) {
        ...
        if (search != null && key != Keyboard.KEY_ESCAPE) { search.keyPressed(character, key); return; }
    ...
    @Override public void render(int mouseX, int mouseY, float tickDelta) {
        renderBackground();
        drawCenteredTextWithShadow(textRenderer, "Jammarr", width / 2, 12, 0xFFFFFF);
        ...
        drawCenteredTextWithShadow(textRenderer, trim(now, width - 20), width / 2, 24,
        ...
        if (!empty(shownNotice)) drawCenteredTextWithShadow(textRenderer, trim(shownNotice, width - 20), width / 2, height - 40, 0xFFB36B);
        if (requestPending) drawCenteredTextWithShadow(textRenderer, "Searching...", width / 2, height / 2, 0xA0D8FF);
        if (view.browseKind != null && view != View.QUEUE) {
            drawCenteredTextWithShadow(textRenderer, "+ Queue   R Radio   M Mix   A Adventure", width / 2, 56, 0x9FAFCF);
        }
        ...
        for (Object item : buttons) {
            ...
            if (tooltip != null && button.isMouseOver(minecraft, mouseX, mouseY)) {
    ...
    private void drawNow(int left, int top, int panel) {
        ...
        drawCenteredTextWithShadow(textRenderer, empty(value.title()) ? "Nothing playing" : value.title(), width / 2, top + 12, 0xFFFFFF);
        if (!empty(value.artist())) drawCenteredTextWithShadow(textRenderer, value.artist(), width / 2, top + 28, 0xCFCFCF);
        if (!empty(value.sourceName())) drawCenteredTextWithShadow(textRenderer, "Source: " + value.sourceName(), width / 2, top + 44, 0xA0D8FF);
        drawCenteredTextWithShadow(textRenderer, "Audio: " + state.audioStatus(), width / 2, top + 66,
    ...
    private String trim(String value, int pixels) {
        int maximum = Math.max(10, pixels);
        if (textRenderer.getWidth(value) <= maximum) return value;
        String suffix = "...";
        int end = value.length();
        while (end > 0 && textRenderer.getWidth(value.substring(0, end) + suffix) > maximum) end--;
