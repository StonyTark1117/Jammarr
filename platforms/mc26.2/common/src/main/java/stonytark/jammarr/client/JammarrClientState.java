...
public final class JammarrClientState {
    ...
    public void accept(JammarrMessage payload) {
        ...
        if (payload instanceof JammarrPayloads.OpenScreen) {
            ...
            if (minecraft.gui.screen() instanceof JammarrScreen screen) {
            ...
            if (minecraft.gui.screen() instanceof JammarrScreen screen) screen.stationChanged();
        } else if (payload instanceof JammarrPayloads.AdventurePreview value) {
            adventurePreview = value;
            if (minecraft.gui.screen() instanceof JammarrScreen screen) screen.adventurePreviewChanged();
            ...
            if (minecraft.player != null) minecraft.player.sendSystemMessage(net.minecraft.network.chat.Component.literal("Jammarr: " + value.message()));
            refreshScreen(minecraft);
            if (minecraft.gui.screen() instanceof JammarrScreen screen) screen.requestFailed();
        }
    }
    ...
    private void probeScreenRendering() {
        ...
        if (unsupportedServer) {
            ...
            if (!(minecraft.gui.screen() instanceof JammarrScreen)) {
                acceptanceScreenTicks = 0;
                if (minecraft.player != null) minecraft.setScreenAndShow(new JammarrScreen(this));
                return;
            }
        ...
        if (!acceptanceScreenVerified) {
            ...
            minecraft.setScreenAndShow(new JammarrClientConfigScreen(minecraft.gui.screen()));
            return;
        }
        if (!(minecraft.gui.screen() instanceof JammarrClientConfigScreen)) {
    ...
    private static void refreshScreen(Minecraft minecraft) { if (minecraft.gui.screen() instanceof JammarrScreen screen) screen.resultsChanged(); }
