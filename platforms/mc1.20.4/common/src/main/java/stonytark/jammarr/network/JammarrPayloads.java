...
public final class JammarrPayloads {
    ...
    public static JammarrMessage read(ResourceLocation id, FriendlyByteBuf buffer) {
        if (OpenScreen.ID.equals(id)) return new OpenScreen();
        if (ClientHello.ID.equals(id)) return ClientHello.read(buffer);
        if (ServerHello.ID.equals(id)) return ServerHello.read(buffer);
        if (TimeSyncRequest.ID.equals(id)) return TimeSyncRequest.read(buffer);
        if (TimeSyncResponse.ID.equals(id)) return TimeSyncResponse.read(buffer);
        if (BrowseRequest.ID.equals(id)) return BrowseRequest.read(buffer);
        if (BrowseResults.ID.equals(id)) return BrowseResults.read(buffer);
        if (QueueRequest.ID.equals(id)) return QueueRequest.read(buffer);
        if (ControlRequest.ID.equals(id)) return ControlRequest.read(buffer);
        if (StationRequest.ID.equals(id)) return StationRequest.read(buffer);
        if (ChunkRequest.ID.equals(id)) return ChunkRequest.read(buffer);
        if (ChunkAcknowledgement.ID.equals(id)) return ChunkAcknowledgement.read(buffer);
        if (AudioHealth.ID.equals(id)) return AudioHealth.read(buffer);
        if (ManifestRequest.ID.equals(id)) return ManifestRequest.read(buffer);
        if (AudioManifest.ID.equals(id)) return AudioManifest.read(buffer);
        if (AudioChunk.ID.equals(id)) return AudioChunk.read(buffer);
        if (PlaybackState.ID.equals(id)) return PlaybackState.read(buffer);
        if (StationState.ID.equals(id)) return StationState.read(buffer);
        if (AdventurePreview.ID.equals(id)) return AdventurePreview.read(buffer);
        if (ErrorMessage.ID.equals(id)) return ErrorMessage.read(buffer);
        throw new DecoderException("Unknown Jammarr payload id: " + id);
    }
