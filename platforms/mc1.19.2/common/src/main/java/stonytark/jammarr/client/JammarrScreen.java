...
public final class JammarrScreen extends Screen {
    ...
    private void addSearch(int left, int top, int panelWidth) {
        ...
        Button go = described(CompatButton.builder(Component.translatable("jammarr.screen.go"), b -> request(0)).bounds(left + panelWidth - 122, top, 58, 20).build(), "Run this search"); go.active = !requestPending; addRenderableWidget(go);
        Button clear = CompatButton.builder(Component.translatable("jammarr.screen.clear"), b -> { searchQuery = ""; search.setValue(""); request(0); })
    ...
    private void addNowPlaying(int left, int top, int panelWidth) {
        ...
        if (state.audioState() == AudioPlaybackState.ERROR)
            addRenderableWidget(described(CompatButton.builder(Component.translatable("jammarr.screen.retry_audio"), b -> state.retryAudio()).bounds(left + panelWidth / 2 - 48, top + 92, 96, 20).build(), "Retry local audio playback after an error"));
    }
    ...
    private void addStations(int left, int top, int panelWidth) {
        ...
        Button autoplay = CompatButton.builder(Component.literal("Autoplay: " + (station.autoplayEnabled() ? "On" : "Off")), b ->
        ...
        addRenderableWidget(described(CompatButton.builder(Component.translatable("jammarr.screen.library_shuffle"), b ->
        ...
        Button stop = CompatButton.builder(Component.translatable("jammarr.screen.stop_station"), b ->
        ...
        for (int i = 0; i < Math.min(5, mixSeeds.size()); i++) {
            ...
            addRenderableWidget(described(CompatButton.builder(Component.literal("×"), b -> { mixSeeds.remove(index); rebuildWidgets(); }).bounds(left + panelWidth - 30, y, 30, 20).build(), "Remove this Sonic Mix seed"));
        }
        int actionY = top + 99 + Math.min(5, mixSeeds.size()) * 22;
        Button start = CompatButton.builder(Component.translatable("jammarr.screen.start"), b ->
        ...
        Button startNow = CompatButton.builder(Component.translatable("jammarr.screen.start_now"), b -> confirmStartNow(JammarrPayloads.StationType.SONIC_MIX, mixSeeds))
        ...
        addRenderableWidget(described(CompatButton.builder(Component.translatable("jammarr.screen.clear_builder"), b -> { mixSeeds.clear(); rebuildWidgets(); })
    ...
    private void addAdventure(int left, int top, int panelWidth) {
        ...
        for (int i = 0; i < adventureWaypoints.size(); i++) {
            ...
            Button up = described(CompatButton.builder(Component.literal("↑"), b -> moveWaypoint(index, -1)).bounds(left + panelWidth - 90, y, 28, 20).build(), "Move this Adventure waypoint earlier"); up.active = i > 0; addRenderableWidget(up);
            Button down = described(CompatButton.builder(Component.literal("↓"), b -> moveWaypoint(index, 1)).bounds(left + panelWidth - 60, y, 28, 20).build(), "Move this Adventure waypoint later"); down.active = i + 1 < adventureWaypoints.size(); addRenderableWidget(down);
            addRenderableWidget(described(CompatButton.builder(Component.literal("×"), b -> { adventureWaypoints.remove(index); rebuildWidgets(); }).bounds(left + panelWidth - 30, y, 30, 20).build(), "Remove this Adventure waypoint"));
        }
        int actionsY = listTop + adventureWaypoints.size() * 22;
        Button preview = CompatButton.builder(Component.translatable("jammarr.screen.preview"), b -> stationRequest(JammarrPayloads.StationAction.PREVIEW_ADVENTURE,
        ...
        Button start = CompatButton.builder(Component.translatable("jammarr.screen.start"), b -> stationRequest(JammarrPayloads.StationAction.START,
        ...
        Button startNow = CompatButton.builder(Component.translatable("jammarr.screen.start_now"), b -> confirmStartNow(JammarrPayloads.StationType.SONIC_ADVENTURE, adventureWaypoints))
        ...
        addRenderableWidget(described(CompatButton.builder(Component.translatable("jammarr.screen.clear_builder"), b -> { adventureWaypoints.clear(); rebuildWidgets(); })
    ...
    private void addAdventureSearchResults(int left, int top, int panelWidth) {
        ...
        for (int i = 0; i < rows && i < results.items().size(); i++) {
            ...
            Button add = CompatButton.builder(Component.literal("A"), b -> addAdventure(item)).bounds(left + panelWidth - 36, y, 36, 20).build();
    ...
    private void addQueueControls(int left, int panelWidth, int y, int index, JammarrPayloads.QueueEntry entry) {
        int x = left + panelWidth - 88; Button up = described(CompatButton.builder(Component.literal("↑"), b -> control(JammarrPayloads.ControlAction.MOVE_UP, index, entry.key())).bounds(x, y, 28, 20).build(), "Move this manual request earlier");
        Button down = described(CompatButton.builder(Component.literal("↓"), b -> control(JammarrPayloads.ControlAction.MOVE_DOWN, index, entry.key())).bounds(x + 30, y, 28, 20).build(), "Move this manual request later");
        ...
        addRenderableWidget(up); addRenderableWidget(down); addRenderableWidget(described(CompatButton.builder(Component.literal("×"), b -> control(JammarrPayloads.ControlAction.REMOVE, index, entry.key())).bounds(x + 60, y, 28, 20).build(), "Remove this manual request"));
    }
    ...
    private void addAction(String label, String tooltip, int x, int y, Button.OnPress press) {
        Button value = CompatButton.builder(Component.literal(label), press).bounds(x, y, 28, 20).build(); described(value, tooltip); addRenderableWidget(value);
    }

    private void addBottomControls(int left, int panelWidth) {
        int bottom = height - 27;
        addRenderableWidget(described(CompatButton.builder(Component.translatable(JammarrSettings.enabled() ? "jammarr.screen.mute" : "jammarr.screen.unmute"), b -> {
        ...
        if (state.playback().operator()) {
            addRenderableWidget(described(CompatButton.builder(Component.translatable(state.playback().paused() ? "jammarr.screen.resume" : "jammarr.screen.pause"), b ->
            ...
            addRenderableWidget(described(CompatButton.builder(Component.translatable("jammarr.screen.skip"), b -> control(JammarrPayloads.ControlAction.SKIP, -1)).bounds(left + 288, bottom, 58, 20).build(), "Skip the current track for everyone"));
            boolean armed = System.currentTimeMillis() < clearArmedUntil;
            addRenderableWidget(described(CompatButton.builder(Component.translatable(armed ? "jammarr.screen.confirm" : "jammarr.screen.clear"), b -> {
    ...
    private void addPaging(int left, int panelWidth) {
        ...
        if (results.page() > 0) { Button previous = described(CompatButton.builder(Component.literal("<"), b -> request(results.page() - 1)).bounds(left + panelWidth - 70, bottom, 32, 20).build(), "Show the previous results page"); previous.active = !requestPending; addRenderableWidget(previous); }
        if (results.hasMore()) { Button next = described(CompatButton.builder(Component.literal(">"), b -> request(results.page() + 1)).bounds(left + panelWidth - 34, bottom, 32, 20).build(), "Show the next results page"); next.active = !requestPending; addRenderableWidget(next); }
    }

    private void addTab(int x, int y, int width, View candidate) {
        Button button = CompatButton.builder(Component.translatable(candidate.label), b -> {
    ...
    @Override public void render(PoseStack graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (requestPending) drawCenteredString(graphics, font, Component.translatable("jammarr.screen.searching"), width / 2, height / 2, 0xA0D8FF);
        else if (queuePending) drawCenteredString(graphics, font, Component.translatable("jammarr.screen.queuing"), width / 2, height / 2, 0xA0D8FF);
        else if (playing.status() == JammarrPayloads.PlaybackStatus.PLEX_OFFLINE && notice.isBlank()) drawCenteredString(graphics, font, Component.translatable("jammarr.screen.plex_unavailable"), width / 2, height / 2, 0xFF7777);
    ...
    private <T extends AbstractWidget> T described(T widget, String description) { tooltips.put(widget, Component.literal(description)); return widget; }
    ...
    private Button disabled(String value, int x, int y, int width) { Button button = CompatButton.builder(Component.literal(trim(value, width - 12)), b -> {}).bounds(x, y, width, 20).build(); button.active = false; return button; }
    ...
    private final class VolumeSlider extends AbstractSliderButton {
        private VolumeSlider(int x, int y, int width, int height) { super(x, y, width, height, Component.empty(), JammarrSettings.volume()); described(this, "Set Jammarr volume on this client only"); updateMessage(); }
