...
public final class LegacyClientConfigScreen extends GuiScreen {
    ...
    @Override public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRenderer, "Jammarr Client Settings", width / 2, height / 2 - 66, 0xFFFFFF);
        drawCenteredString(fontRenderer, "These settings affect only this Minecraft client", width / 2, height / 2 - 50, 0xA0D8FF);
        drawCenteredString(fontRenderer, "Volume " + Math.round(JammarrSettings.volume() * 100.0) + "%", width / 2, height / 2 + 6, 0xCFCFCF);
