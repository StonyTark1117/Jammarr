...
public final class LegacySavedData extends WorldSavedData implements PlaybackStore {
    ...
    public void readFromNBT(NBTTagCompound tag) {
        ...
        NBTTagList queueTags = tag.getTagList("queue");
        for (int index = 0; index < Math.min(MAX_QUEUE, queueTags.tagCount()); index++) {
            QueueTrack track = readTrack((NBTTagCompound) queueTags.tagAt(index));
        ...
        if (schema < 2) {
            ...
            if (tag.hasKey("station")) station = readStation(tag.getCompoundTag("station"));
            autoplayEnabled = tag.getBoolean("autoplayEnabled");
            NBTTagList historyTags = tag.getTagList("history");
            ...
            for (int index = first; index < historyTags.tagCount(); index++) {
                QueueTrack track = readTrack((NBTTagCompound) historyTags.tagAt(index));
    ...
    private static StationModels.StationDefinition readStation(NBTTagCompound tag) {
        ...
        NBTTagList values = tag.getTagList("seeds");
        for (int index = 0; index < Math.min(5, values.tagCount()); index++) {
            NBTTagCompound value = (NBTTagCompound) values.tagAt(index);
