...
/** Compact eight-tab Forge 1.7.10 screen with full shared-control flows. */
final class LegacyScreen extends GuiScreen {
    ...
    @Override public void initGui() {
        ...
        for (int index = 0; index < View.values().length; index++) {
            ...
            GuiButton tab = button(TAB_BASE + index, left + tabWidth * index, 35,
                    index == View.values().length - 1 ? panel - tabWidth * index : tabWidth, 20, candidate.label);
            tab.enabled = candidate != view;
        }
        int top = 65;
        if (view == View.SEARCH || view == View.ADVENTURE) {
            search = new GuiTextField(fontRenderer, left, top, panel - 66, 20);
    ...
    private void addAdventureButtons(int left, int top, int panel) {
        ...
        GuiButton preview = button(ADVENTURE_PREVIEW, left, actions, 76, 20, "Preview");
        GuiButton start = button(ADVENTURE_START, left + 82, actions, 76, 20, "Start");
        GuiButton startNow = button(ADVENTURE_START_NOW, left + 164, actions, 94, 20, "Start Now");
        preview.enabled = start.enabled = startNow.enabled = waypoints.size() >= 2;
            ...
            if (view == View.QUEUE) {
                ...
                if (queueIndex < state.playback().queue().size()) {
                    ...
                    if (entry.editable() && state.playback().operator()) {
                        GuiButton up = button(UP_BASE + row, left + panel - 84, y, 26, 20, "^");
                        GuiButton down = button(DOWN_BASE + row, left + panel - 56, y, 26, 20, "v");
                        button(REMOVE_BASE + row, left + panel - 28, y, 26, 20, "X");
                        up.enabled = queueIndex > 0 && state.playback().queue().get(queueIndex - 1).editable();
                        down.enabled = queueIndex + 1 < state.playback().queue().size()
    ...
    private GuiButton button(int id, int x, int y, int width, int height, String label) {
        GuiButton value = new GuiButton(id, x, y, width, height, label);
    ...
    @Override protected void actionPerformed(GuiButton button) {
        ...
        if (button.id == MUTE) {
            ...
            state.listeningChanged(); initGui(); return;
        }
        if (button.id == VOLUME_DOWN || button.id == VOLUME_UP) {
            double delta = button.id == VOLUME_UP ? 0.1 : -0.1;
            JammarrSettings.volume(JammarrSettings.volume() + delta); JammarrSettings.saveVolume(); initGui(); return;
        }
        if (button.id == RETRY) { state.retryAudio(); initGui(); return; }
        if (button.id == PAUSE) {
        ...
        if (button.id == CLEAR) {
            ...
            } else { clearArmedUntil = System.currentTimeMillis() + 5_000L; notice = "Press Clear again to confirm"; initGui(); }
            return;
        }
        ...
        if (button.id == ADVENTURE_START_NOW) {
            if (System.currentTimeMillis() < startNowArmedUntil) {
                ...
                notice = "Press Start Now again to replace current playback"; initGui();
            }
            return;
        }
        if (button.id == ADVENTURE_CLEAR) { waypoints.clear(); initGui(); return; }
    ...
    @Override public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRenderer, "Jammarr", width / 2, 12, 0xFFFFFF);
        ...
        drawCenteredString(fontRenderer, trim(now, width - 20), width / 2, 24,
        ...
        if (search != null) { search.drawTextBox(); top += 26; }
        ...
        if (!empty(shownNotice)) drawCenteredString(fontRenderer, trim(shownNotice, width - 20), width / 2, height - 40, 0xFFB36B);
        if (requestPending) drawCenteredString(fontRenderer, "Searching...", width / 2, height / 2, 0xA0D8FF);
        if (view.browseKind != null && view != View.QUEUE) {
            drawCenteredString(fontRenderer, "+ Queue   R Radio   M Mix   A Adventure", width / 2, 56, 0x9FAFCF);
        }
        drawString(fontRenderer, "Volume " + Math.round(JammarrSettings.volume() * 100.0) + "%", left + 134, height - 21, 0xCFCFCF);
        super.drawScreen(mouseX, mouseY, partialTicks);
        for (Object item : buttonList) {
            GuiButton button = (GuiButton)item;
            String tooltip = tooltip(button.id);
            if (tooltip != null && button.func_82252_a()) {
    ...
    private void drawNow(int left, int top, int panel) {
        ...
        drawCenteredString(fontRenderer, empty(value.title()) ? "Nothing playing" : value.title(), width / 2, top + 12, 0xFFFFFF);
        if (!empty(value.artist())) drawCenteredString(fontRenderer, value.artist(), width / 2, top + 28, 0xCFCFCF);
        if (!empty(value.sourceName())) drawCenteredString(fontRenderer, "Source: " + value.sourceName(), width / 2, top + 44, 0xA0D8FF);
        drawCenteredString(fontRenderer, "Audio: " + state.audioStatus(), width / 2, top + 66,
    ...
    private void drawStations(int left, int top, int panel) {
        ...
        drawString(fontRenderer, "Sonic: " + value.capability().name().toLowerCase() + " - "
                + value.capabilityMessage(), left, top, 0xA0D8FF);
        drawString(fontRenderer, value.active() && value.stationType() != StationModels.StationType.SONIC_ADVENTURE
                ? "Active: " + value.name() : "No general station active", left, top + 16, 0xCFCFCF);
        drawString(fontRenderer, "Mix seeds: " + mixSeeds.size() + "/5", left, top + 28, 0xCFCFCF);
        int y = top + 72;
        for (StatePackets.QueueEntry entry : value.preview()) {
            drawString(fontRenderer, trim(entry.title() + " - " + entry.artist(), panel), left, y, 0xBFCFFF); y += 14;
        }
    }

    private void drawAdventure(int left, int top, int panel) {
        drawString(fontRenderer, "Sonic Adventure waypoints (2-5)", left, top, 0xA0D8FF);
        for (int index = 0; index < waypoints.size(); index++) {
            StationModels.StationSeed seed = waypoints.get(index);
            drawString(fontRenderer, (index + 1) + ". " + trim(seed.title() + " - " + seed.subtitle(), panel - 30), left, top + 18 + index * 20, 0xCFCFCF);
        }
        int resultsTop = top + Math.min(5, waypoints.size()) * 20 + 70;
        if (!empty(state.adventure().message())) {
            drawString(fontRenderer, state.adventure().message(), left, resultsTop, 0xBFCFFF); resultsTop += 14;
        }
    ...
    private void drawResults(int left, int top, int panel) {
        ...
        for (int row = 0; row < Math.min(MAX_VISIBLE_ROWS, results.items().size()); row++) {
            ...
            drawString(fontRenderer, trim(prefix + item.title()
                    + (empty(item.subtitle()) ? "" : " - " + item.subtitle()), panel - 120),
                    left, top + row * 22 + 6, 0xE0E0E0);
        }
    }

    private String trim(String value, int pixels) { return fontRenderer.trimStringToWidth(value, Math.max(10, pixels)); }

    private void drawTooltip(String value, int mouseX, int mouseY) {
        int tooltipWidth = fontRenderer.getStringWidth(value);
        int x = Math.min(mouseX + 10, width - tooltipWidth - 6);
        int y = Math.min(mouseY + 10, height - 16);
        drawGradientRect(x - 3, y - 3, x + tooltipWidth + 3, y + 11, 0xF0100010, 0xF0100010);
        drawString(fontRenderer, value, x, y, 0xFFFFFF);
    }
