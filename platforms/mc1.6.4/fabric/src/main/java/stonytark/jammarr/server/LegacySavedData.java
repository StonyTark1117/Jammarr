...
/** Schema-4 world state stored through Forge 1.6.4 PersistentState. */
public final class LegacySavedData extends PersistentState implements PlaybackStore {
    ...
    public static LegacySavedData get(MinecraftServer server) {
        ...
        PersistentStateManager storage = overworld.persistentStateManager;
        LegacySavedData data = (LegacySavedData) storage.getOrCreate(LegacySavedData.class, DATA_NAME);
        if (data == null) {
            data = new LegacySavedData(DATA_NAME);
            storage.replace(DATA_NAME, data);
            data.markDirty();
        }
        return data;
    }
    ...
    /**
     * PlaybackStore deliberately avoids PersistentState.markDirty's MCP name:
     * that inherited method is reobfuscated in the production 1.6.4 JAR.
     */
    ...
    public void fromNbt(NbtCompound tag) {
        ...
        NbtList queueTags = tag.getList("queue");
        for (int index = 0; index < Math.min(MAX_QUEUE, queueTags.size()); index++) {
            QueueTrack track = readTrack((NbtCompound) queueTags.getItem(index));
        ...
        if (schema < 2) {
            ...
            if (tag.contains("station")) station = readStation(tag.getCompound("station"));
            autoplayEnabled = tag.getBoolean("autoplayEnabled");
            NbtList historyTags = tag.getList("history");
            ...
            for (int index = first; index < historyTags.size(); index++) {
                QueueTrack track = readTrack((NbtCompound) historyTags.getItem(index));
    ...
    public void toNbt(NbtCompound tag) {
        ...
        for (int index = 0; index < Math.min(MAX_QUEUE, queue.size()); index++) queueTags.addItem(writeTrack(queue.get(index)));
        ...
        for (int index = first; index < history.size(); index++) historyTags.addItem(writeTrack(history.get(index)));
    ...
    private static StationModels.StationDefinition readStation(NbtCompound tag) {
        ...
        NbtList values = tag.getList("seeds");
        for (int index = 0; index < Math.min(5, values.size()); index++) {
            NbtCompound value = (NbtCompound) values.getItem(index);
