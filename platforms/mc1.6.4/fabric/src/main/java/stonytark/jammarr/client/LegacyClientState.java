...
final class LegacyClientState implements LegacyNetwork.ClientListener {
    ...
    @Override public void accept(LegacyPacketTypes.Type<?> type, Object message) {
        ...
        if (type == LegacyPacketTypes.OPEN_SCREEN) {
            ...
            if (hello.protocolVersion() != ProtocolLimits.clientHelloVersion() && minecraft.method_2960() != null) {
                minecraft.method_2960().method_1205().disconnect(
                        "Jammarr protocol mismatch: server requires version " + hello.protocolVersion());
            } else {
            ...
            if (minecraft.inGameHud != null) minecraft.inGameHud.getChatHud().addMessage("Jammarr: " + notice);
    ...
    void tick() {
        ...
        if (serverHelloReceived && ProtocolLimits.commandProbeEnabled() && !commandProbeSent
                && MinecraftClient.getInstance().field_3805 != null) {
            commandProbeSent = true;
            MinecraftClient.getInstance().field_3805.method_1262("/jammarr status");
            MinecraftClient.getInstance().field_3805.method_1262("/jammarr diagnostics");
    ...
    void operatorCommandProbe() {
        if (!ProtocolLimits.commandProbeEnabled() || operatorProbeSent
                || MinecraftClient.getInstance().field_3805 == null) return;
    ...
    private void requestTimeSync() {
        if (MinecraftClient.getInstance().method_2960() == null) return;
        long now = System.currentTimeMillis();
    ...
    private LegacyScreen screen() {
        return MinecraftClient.getInstance().currentScreen instanceof LegacyScreen
                ? (LegacyScreen) MinecraftClient.getInstance().currentScreen : null;
    }
