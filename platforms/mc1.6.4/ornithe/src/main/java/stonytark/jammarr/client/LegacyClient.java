...
public final class LegacyClient implements ClientModInitializer {
    ...
    @Override public void initClient() {
        ...
        KeybindEvents.REGISTER_KEYBINDS.register(() ->
                KeybindRegistry.register(open, "key.categories.jammarr"));
        ...
        ClientConnectionEvents.DISCONNECT.register(context -> {
            TextComponent reason = context.disconnectReason();
            Jammarr.LOGGER.info("Client disconnected with reason: {}",
                    reason == null ? "Disconnected" : reason.buildString());
    ...
    public static void acceptanceChat(Text text) {
        ...
        String message = text.buildString(false);
