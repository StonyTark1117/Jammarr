...
public final class JammarrScreen extends Screen {
    ...
    @Override protected void init() {
        clearWidgets(); tooltips.clear(); int panelWidth = Math.min(760, width - 16), left = (width - panelWidth) / 2;
    ...
    private void addSearch(int left, int top, int panelWidth) {
        search = addRenderableWidget(new EditBox(font, left, top, panelWidth - 126, 20, stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.search")));
        search.setMaxLength(128); search.setValue(searchQuery); search.setResponder(value -> searchQuery = value);
        Button go = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.go"), b -> request(0)).bounds(left + panelWidth - 122, top, 58, 20).build(), "Run this search"); go.active = !requestPending; addRenderableWidget(go);
        Button clear = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.clear"), b -> { searchQuery = ""; search.setValue(""); request(0); })
    ...
    private void addNowPlaying(int left, int top, int panelWidth) {
        ...
        String title = playing.title().isBlank() ? stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.nothing_playing").getString() : playing.title();
        Button titleButton = disabled(trim(title, panelWidth - 20), left, top + 18, panelWidth); addRenderableWidget(titleButton);
        if (font.width(title) > panelWidth - 20) described(titleButton, title);
        ...
        if (state.audioState() == AudioPlaybackState.ERROR)
            addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.retry_audio"), b -> state.retryAudio()).bounds(left + panelWidth / 2 - 48, top + 92, 96, 20).build(), "Retry local audio playback after an error"));
    }
    ...
    private void addStations(int left, int top, int panelWidth) {
        ...
        Button autoplay = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("Autoplay: " + (station.autoplayEnabled() ? "On" : "Off")), b ->
        ...
        addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.library_shuffle"), b ->
        ...
        Button stop = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.stop_station"), b ->
        ...
        for (int i = 0; i < Math.min(5, mixSeeds.size()); i++) {
            ...
            addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("×"), b -> { mixSeeds.remove(index); rebuildWidgets(); }).bounds(left + panelWidth - 30, y, 30, 20).build(), "Remove this Sonic Mix seed"));
        }
        int actionY = top + 99 + Math.min(5, mixSeeds.size()) * 22;
        Button start = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.start"), b ->
        ...
        Button startNow = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.start_now"), b -> confirmStartNow(JammarrPayloads.StationType.SONIC_MIX, mixSeeds))
        ...
        addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.clear_builder"), b -> { mixSeeds.clear(); rebuildWidgets(); })
    ...
    private void addAdventure(int left, int top, int panelWidth) {
        ...
        for (int i = 0; i < adventureWaypoints.size(); i++) {
            ...
            Button up = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("↑"), b -> moveWaypoint(index, -1)).bounds(left + panelWidth - 90, y, 28, 20).build(), "Move this Adventure waypoint earlier"); up.active = i > 0; addRenderableWidget(up);
            Button down = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("↓"), b -> moveWaypoint(index, 1)).bounds(left + panelWidth - 60, y, 28, 20).build(), "Move this Adventure waypoint later"); down.active = i + 1 < adventureWaypoints.size(); addRenderableWidget(down);
            addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("×"), b -> { adventureWaypoints.remove(index); rebuildWidgets(); }).bounds(left + panelWidth - 30, y, 30, 20).build(), "Remove this Adventure waypoint"));
        }
        int actionsY = listTop + adventureWaypoints.size() * 22;
        Button preview = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.preview"), b -> stationRequest(JammarrPayloads.StationAction.PREVIEW_ADVENTURE,
        ...
        Button start = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.start"), b -> stationRequest(JammarrPayloads.StationAction.START,
        ...
        Button startNow = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.start_now"), b -> confirmStartNow(JammarrPayloads.StationType.SONIC_ADVENTURE, adventureWaypoints))
        ...
        addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.clear_builder"), b -> { adventureWaypoints.clear(); rebuildWidgets(); })
    ...
    private void addAdventureSearchResults(int left, int top, int panelWidth) {
        ...
        for (int i = 0; i < rows && i < results.items().size(); i++) {
            ...
            Button add = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("A"), b -> addAdventure(item)).bounds(left + panelWidth - 36, y, 36, 20).build();
            described(add, "Add as Adventure waypoint"); add.active = adventureWaypoints.size() < 5; addRenderableWidget(add);
        }
    }
    ...
    private void addResults(int left, int contentTop, int panelWidth) {
        ...
        for (int row = 0; row < rows && row + rowOffset < items.size(); row++) {
            ...
            if (font.width(label) > panelWidth - controlsWidth - 20) described(itemButton, label); addRenderableWidget(itemButton);
    ...
    private void addQueueControls(int left, int panelWidth, int y, int index, JammarrPayloads.QueueEntry entry) {
        int x = left + panelWidth - 88; Button up = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("↑"), b -> control(JammarrPayloads.ControlAction.MOVE_UP, index, entry.key())).bounds(x, y, 28, 20).build(), "Move this manual request earlier");
        Button down = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("↓"), b -> control(JammarrPayloads.ControlAction.MOVE_DOWN, index, entry.key())).bounds(x + 30, y, 28, 20).build(), "Move this manual request later");
        ...
        addRenderableWidget(up); addRenderableWidget(down); addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("×"), b -> control(JammarrPayloads.ControlAction.REMOVE, index, entry.key())).bounds(x + 60, y, 28, 20).build(), "Remove this manual request"));
    }
    ...
    private void addAction(String label, String tooltip, int x, int y, Button.OnPress press) {
        Button value = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal(label), press).bounds(x, y, 28, 20).build(); described(value, tooltip); addRenderableWidget(value);
    }

    private void addBottomControls(int left, int panelWidth) {
        int bottom = height - 27;
        addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable(JammarrSettings.enabled() ? "jammarr.screen.mute" : "jammarr.screen.unmute"), b -> {
        ...
        if (state.playback().operator()) {
            addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable(state.playback().paused() ? "jammarr.screen.resume" : "jammarr.screen.pause"), b ->
            ...
            addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.skip"), b -> control(JammarrPayloads.ControlAction.SKIP, -1)).bounds(left + 288, bottom, 58, 20).build(), "Skip the current track for everyone"));
            boolean armed = System.currentTimeMillis() < clearArmedUntil;
            addRenderableWidget(described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable(armed ? "jammarr.screen.confirm" : "jammarr.screen.clear"), b -> {
                if (System.currentTimeMillis() < clearArmedUntil) { clearArmedUntil = 0; control(JammarrPayloads.ControlAction.CLEAR, -1); }
                else { clearArmedUntil = System.currentTimeMillis() + 5_000; screenNotice = stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.confirm_clear_notice").getString(); rebuildWidgets(); }
    ...
    private void addPaging(int left, int panelWidth) {
        ...
        if (results.page() > 0) { Button previous = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal("<"), b -> request(results.page() - 1)).bounds(left + panelWidth - 70, bottom, 32, 20).build(), "Show the previous results page"); previous.active = !requestPending; addRenderableWidget(previous); }
        if (results.hasMore()) { Button next = described(CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal(">"), b -> request(results.page() + 1)).bounds(left + panelWidth - 34, bottom, 32, 20).build(), "Show the next results page"); next.active = !requestPending; addRenderableWidget(next); }
    }

    private void addTab(int x, int y, int width, View candidate) {
        Button button = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.translatable(candidate.label), b -> {
    ...
    private void activate(JammarrPayloads.MediaItem item) {
        if (queuePending) return; queuePending = true; pendingQueueKey = item.key(); screenNotice = stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.queuing").getString();
    ...
    private void request(int page) {
        ...
        if ((view == View.SEARCH || view == View.ADVENTURE) && searchQuery.length() < 2) {
            requestPending = false; screenNotice = stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.short_query").getString(); state.clearBrowse(view.browseKind, searchQuery); rebuildWidgets(); return;
        }
    ...
    void playbackChanged() { if (!queuePending || state.playback().queue().stream().noneMatch(entry -> entry.key().equals(pendingQueueKey) && entry.source() == JammarrPayloads.PlaybackOrigin.MANUAL)) return; queuePending = false; pendingQueueKey = ""; screenNotice = stonytark.jammarr.compat.MinecraftCompat.translatable("jammarr.screen.queued").getString(); rebuildWidgets(); }
    ...
    @Override public void render(PoseStack graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (!notice.isBlank()) drawCenteredString(graphics, font, trim(notice, width - 24), width / 2, height - 40, 0xFFB36B);
        if (view == View.NOW) drawCenteredString(graphics, font, "Audio: " + state.audioStatus(), width / 2, 187, state.audioState() == AudioPlaybackState.ERROR ? 0xFF7777 : 0xA0D8FF);
    ...
    private Button disabled(String value, int x, int y, int width) { Button button = CompatButton.builder(stonytark.jammarr.compat.MinecraftCompat.literal(trim(value, width - 12)), b -> {}).bounds(x, y, width, 20).build(); button.active = false; return button; }
    ...
    private static String statusLabel(JammarrPayloads.PlaybackState state) { return stonytark.jammarr.compat.MinecraftCompat.translatable(switch (state.status()) { case IDLE -> "jammarr.status.idle"; case PREPARING -> "jammarr.status.preparing"; case PLAYING -> "jammarr.status.playing"; case PAUSED -> "jammarr.status.paused"; case PLEX_OFFLINE -> "jammarr.status.plex_offline"; }).getString(); }
    ...
    private final class VolumeSlider extends AbstractSliderButton {
        private VolumeSlider(int x, int y, int width, int height) { super(x, y, width, height, stonytark.jammarr.compat.MinecraftCompat.empty(), JammarrSettings.volume()); described(this, "Set Jammarr volume on this client only"); updateMessage(); }
