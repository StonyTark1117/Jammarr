...
public final class JammarrCommands {
    ...
    private static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralArgumentBuilder<CommandSourceStack> root = Commands.literal("jammarr").requires(source -> { try { return JammarrServer.instance().accepted(source.getPlayerOrException()); } catch (com.mojang.brigadier.exceptions.CommandSyntaxException ignored) { return true; } })
                ...
                .then(Commands.literal("status").executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); c.getSource().sendSuccess(stonytark.jammarr.compat.MinecraftCompat.literal(p == null ? "Jammarr is unavailable" : p.status()), false); return 1; }));
        ...
        root.then(Commands.literal("reload").requires(s -> s.hasPermission(JammarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); if (p != null) p.validatePlex(); c.getSource().sendSuccess(stonytark.jammarr.compat.MinecraftCompat.literal("Jammarr Plex validation started"), false); return 1; }));
        root.then(Commands.literal("cache").requires(s -> s.hasPermission(JammarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); long bytes = p == null ? 0 : p.cacheSize(); c.getSource().sendSuccess(stonytark.jammarr.compat.MinecraftCompat.literal("Jammarr cache: " + bytes / 1024 / 1024 + " MiB"), false); return 1; }));
        root.then(Commands.literal("diagnostics").requires(s -> s.hasPermission(JammarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); c.getSource().sendSuccess(stonytark.jammarr.compat.MinecraftCompat.literal(p == null ? "Jammarr is unavailable" : p.diagnostics()), false); return 1; }));
        root.then(Commands.literal("station").requires(s -> s.hasPermission(JammarrSettings.operatorPermissionLevel()))
                .then(Commands.literal("status").executes(c -> { GlobalPlayer p = JammarrServer.instance().player(); c.getSource().sendSuccess(stonytark.jammarr.compat.MinecraftCompat.literal(p == null ? "Jammarr is unavailable" : p.stationStatus()), false); return 1; }))
    ...
    private static int station(CommandSourceStack source, JammarrPayloads.StationAction action,
        ...
        if (player == null) { source.sendFailure(stonytark.jammarr.compat.MinecraftCompat.literal("Jammarr is unavailable")); return 0; }
