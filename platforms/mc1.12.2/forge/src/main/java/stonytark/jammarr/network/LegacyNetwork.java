...
/** Forge 1.12.2 SimpleNetworkWrapper adapter for the canonical protocol-6 codecs. */
public final class LegacyNetwork {
    ...
    public void customPacketRegistration(FMLNetworkEvent.CustomPacketRegistrationEvent<?> event) {
        ...
        if ("REGISTER".equals(event.getOperation()) && event.getRegistrations().contains(Jammarr.MOD_ID)) serverAvailable = true;
        if ("UNREGISTER".equals(event.getOperation()) && event.getRegistrations().contains(Jammarr.MOD_ID)) serverAvailable = false;
    }
    ...
    private void handleServer(ServerIncoming incoming) {
        ...
        if (incoming.type == LegacyPacketTypes.CLIENT_HELLO) {
            ...
            if (hello.protocolVersion() != Jammarr.PROTOCOL) {
                ...
                disconnect(player, "Jammarr protocol mismatch: server requires protocol " + Jammarr.PROTOCOL);
                return;
            }
        ...
        if (!capabilities.capable(player.getUniqueID())) {
            disconnect(player, "Jammarr protocol hello is required before play packets");
            return;
        }
    ...
    public static final class ServerboundHandler implements IMessageHandler<LegacyServerboundEnvelope, IMessage> {
        ...
        public IMessage onMessage(LegacyServerboundEnvelope envelope, MessageContext context) {
            ...
            try {
                ...
                disconnect(player, "Malformed Jammarr packet: " + malformed.getMessage());
            }
            return null;
        }
    }
    ...
    private static void disconnect(EntityPlayerMP player, String message) {
